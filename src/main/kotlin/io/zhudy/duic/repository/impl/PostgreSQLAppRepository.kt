...
class PostgreSQLAppRepository(
    ...
    override fun insert(app: App) = Mono.create<Int> {
        val n = transactionTemplate.execute {
            jdbcTemplate.update(
                    "INSERT INTO app(name,profile,description,token,ip_limit,content,v,users,created_at,updated_at) VALUES(:name,:profile,:description,:token,:ipLimit,:content,:v,:users,now(),now())",
                    mapOf(
                            "name" to app.name,
    ...
    override fun findPageByUser(pageable: Pageable, userContext: UserContext) = Mono.zip(
            Flux.create<App> { sink ->
                roTransactionTemplate.execute {
                    jdbcTemplate.query(
                            "SELECT * FROM app WHERE users LIKE CONCAT('%', :email, '%') LIMIT :limit OFFSET :offset",
                            mapOf(
            ...
            Flux.create<App> { sink ->
                roTransactionTemplate.execute {
                    val sql = if (q.isEmpty()) {
                        ...
                        "SELECT * FROM app WHERE TO_TSVECTOR(name || ' ' || profile || ' ' || content) @@ TO_TSQUERY(:q) LIMIT :limit OFFSET :offset"
                    }
            ...
            Mono.create<Int> {
                val c = roTransactionTemplate.execute {
                    val sql = if (q.isEmpty()) {
                        ...
                        "SELECT COUNT(*) FROM app WHERE TO_TSVECTOR(name || ' ' || profile || ' ' || content) @@ TO_TSQUERY(:q)"
                    }
            ...
            Flux.create<App> { sink ->
                roTransactionTemplate.execute {
                    val sql = if (q.isEmpty()) {
                        ...
                        "SELECT * FROM app WHERE users LIKE CONCAT('%', :email, '%') AND TO_TSVECTOR(name || ' ' || profile || ' ' || content) @@ TO_TSQUERY(:q) LIMIT :limit OFFSET :offset"
                    }
                    jdbcTemplate.query(
                            sql,
                            mapOf(
            ...
            Mono.create<Int> {
                val c = roTransactionTemplate.execute {
                    val sql = if (q.isEmpty()) {
                        ...
                        "SELECT COUNT(*) FROM app WHERE users LIKE CONCAT('%', :email, '%') AND TO_TSVECTOR(name || ' ' || profile || ' ' || content) @@ TO_TSQUERY(:q)"
                    }
    ...
    override fun findLast50History(name: String, profile: String) = Flux.create<AppContentHistory> { sink ->
        roTransactionTemplate.execute {
            jdbcTemplate.query(
                    "SELECT * FROM app_history WHERE name=:name AND profile=:profile ORDER BY created_at DESC LIMIT 50",
                    mapOf("name" to name, "profile" to profile)
            ) {
